...
import { createRelayEnvironmentSsr } from '@karma/relay-ssr';

import { GRAPHQL_URL, sessionCookieName } from './config';
import { historyMiddlewares, routeConfig, render } from './router';
...
router.get('/*', async ctx => {
  ...
  const environment = createRelayEnvironmentSsr(relaySsr, GRAPHQL_URL, [
    next => req => {
  ...
  if ('redirect' in result) {
    ctx.redirect(result.redirect.url);
    return;
  }

  const { html, styleTags, helmet } = renderHtml(
  ...
  try {
    ...
    ctx.body = indexHtml({ helmet, assets, styleTags, relayData, html });
  } catch (err) {
